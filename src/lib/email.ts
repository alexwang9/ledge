...
const FROM_EMAIL = process.env.FROM_EMAIL || 'Ledge <onboarding@resend.dev>';
...
export async function sendVerificationCode(email: string, code: string): Promise<boolean> {
  try {
    const { error } = await resend.emails.send({
      from: FROM_EMAIL,
      to: email,
      subject: 'Your Ledge verification code',
      html: `
        <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 480px; margin: 0 auto; padding: 40px 20px;">
          ...
          <p style="color: #666; font-size: 16px; line-height: 1.5; margin-bottom: 24px;">
            Enter this code to complete your sign in to Ledge:
          </p>
